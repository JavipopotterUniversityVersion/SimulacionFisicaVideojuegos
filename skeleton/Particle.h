...
public:
	Particle() : position(new PxTransform({0, 0, 0})), velocity(0, 0, 0), acceleration(0, 0, 0), mass(1.0), lifeTime(10), damping(1.0), lastPosition(0, 0, 0), mode(EULER){
		...
		item = new RenderItem(shape, position, Vector4(1, 1, 0, 1));
	}

	Particle(Vector3 P, Vector3 V, Vector3 A, double T, double d, Mode mass) : position(new PxTransform(P)), velocity(V), acceleration(A), mass(1.0), lifeTime(T), damping(d), lastPosition(0, 0, 0), mode(mass) {
